"""Shared MuJoCo runtime for headless evaluation and web-based viewing."""
...
class MujocoRuntime:
    """Run the exported policy against a MuJoCo model.

    The policy observation, action conversion, and PD loop are deliberately kept
    here so every front end (batch evaluation, native viewer, or mjviser) uses the
    ...
    def __init__(self, model, data, config: DeploymentConfig, policy: OnnxPolicy, mujoco_module: ModuleType) -> None:
        ...
        for name in config.joint_names:
            joint_id = mujoco_module.mj_name2id(model, mujoco_module.mjtObj.mjOBJ_JOINT, f"{name}_joint")
        ...
        if self.sim_steps_per_control < 1 or not np.isclose(
            self.sim_steps_per_control * model.opt.timestep, config.control_dt
        ):
            raise ValueError("MuJoCo timestep must divide the exported control period")
        # Use a guaranteed-valid neutral initialization; front ends set their
        # requested command immediately after construction.
    ...
    def from_paths(cls, policy_dir: str | Path, model_path: str | Path):
        ...
        policy = OnnxPolicy(policy_dir / "policy.onnx")
        model = mujoco.MjModel.from_xml_path(str(Path(model_path).expanduser().resolve()))
        data = mujoco.MjData(model)
        return cls(model, data, config, policy, mujoco)
    ...
    def reset(self, model=None, data=None) -> None:
        """Reset model state and all policy history (mjviser callback compatible)."""
        with self._lock:
    ...
    def _prepare_action(self) -> None:
        ...
        frame = build_proprio(
            self._sensor("imu_gyro"),
            self._sensor("imu_quat"),
            self.command,
            joint_pos,
            joint_vel,
        ...
        self._pending_action = self.policy(self.history.append(frame))[0]
        self._target = self.config.action_to_joint_target(self._pending_action)
        self.policy_steps += 1

    def step_physics(self, model=None, data=None) -> None:
        """Advance one MuJoCo physics step (mjviser ``step_fn`` callback)."""
        with self._lock:
    ...
    def step_control(self) -> None:
        """Advance one policy/control period for deterministic batch evaluation."""
