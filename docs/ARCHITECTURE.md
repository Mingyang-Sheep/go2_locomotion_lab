...
```text
Competition PPO                         HIM
  PPO Loco Teacher                       HIM estimator + actor
  High-Step Loco (future)                HIM Sim2Real adapter
  LBC (future)                              |
          \                               /
...
- `algorithms/him/`: HIM estimator, actor, and RSL-RL PPO extension.
- `evaluation/`: metrics and report serialization shared by every future route.
- `deployment/`: Isaac-independent ONNX observation/action runtime.
